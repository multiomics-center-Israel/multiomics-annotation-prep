...
def main():
    ...
    parser.add_argument("--emit-compound-sets", action="store_true",
                        help="Also write the ID-based compound-set GMT + readable "
                             "table from the same load_source (same KEGG snapshot); "
                             "recorded in the manifest's companion_files")
    ...
    model_path, manifest_path = prepare_mummichog_model(
        ...
        kaas_file=args.kaas,
        emit_compound_sets=args.emit_compound_sets,
    )
